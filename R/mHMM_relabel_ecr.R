...
mHMM_relabel_ecr <- function(
  ...
  emiss_sampler = NULL,
  relabel_train = 100,
  relabel_type = "observed",
  relabel_steps = 1,
  relabel_burnin = 10,
  relabel_group = FALSE,
  relabel_group_iter = NULL
) {
  # Initialize data -----------------------------------
  ...
  if (sum(sapply(s_data, is.factor)) > 0) {
    stop(
      "Your data contains factorial variables, which cannot be used as input in the function mHMM. All variables have to be numerical."
    )
  }
  ...
  for (s in 1:n_subj) {
    subj_data[[s]]$y <- as.matrix(
      s_data[s_data[, 1] == id[s], ][, -1],
      ncol = n_dep
    )
  }
  ypooled <- n_t <- NULL
  ...
  if (data_distr == 'categorical') {
    if (sum(objects(gen) %in% "q_emiss") != 1) {
      stop(
        "The input argument gen should contain the elements q_emiss when modelling categorical observations."
      )
    }
    q_emiss <- gen$q_emiss
    if (length(q_emiss) != n_dep) {
      stop(
        "The lenght of q_emiss specifying the number of output categories for each of the number of dependent variables should equal the number of dependent variables specified in n_dep"
      )
    }
    if (
      sum(
        apply(as.matrix(s_data[, 2:(n_dep + 1)], ncol = n_dep), 2, function(x) {
          length(unique(x))
        }) !=
          q_emiss
      ) >
        ...
        sum(
          apply(as.matrix(s_data[, 2:(n_dep + 1)], ncol = n_dep), 2, max) >
            q_emiss
        ) >
          0
    ) {
  ...
  if (is.null(xx)) {
    ...
    if (!is.list(xx) | length(xx) != n_dep1) {
      stop(
        "If xx is specified, xx should be a list, with the number of elements equal to the number of dependent variables + 1"
      )
    }
    for (i in 1:n_dep1) {
      if (is.null(xx[[i]])) {
        ...
        if (sum(is.na(xx[[i]])) > 1) {
          stop(
            "Currently, missing values (NA) are only allowed in the dependent variables and not in the covariate(s) values xx"
          )
        }
        if (sum(xx[[i]][, 1] != 1)) {
          stop(
            "If xx is specified, the first column in each element of xx has to represent the intercept. That is, a column that only consists of the value 1"
          )
        }
        if (nx[i] > 1) {
          for (j in 2:nx[i]) {
            if (is.factor(xx[[i]][, j])) {
              stop(
                "Factors currently cannot be used as covariates, see help file for alternatives"
              )
            }
            if (
              (length(unique(xx[[i]][, j])) == 2) &
                (sum(xx[[i]][, j] != 0 & xx[[i]][, j] != 1) > 0)
            ) {
              stop(
                "Dichotomous covariates in xx need to be coded as 0 / 1 variables. That is, only conisting of the values 0 and 1"
              )
            }
  ...
  if (is.null(gamma_sampler)) {
    ...
    if (!is.mHMM_pdRW_gamma(gamma_sampler)) {
      stop(
        "The input object specified for gamma_sampler should be from the class mHMM_pdRW_gamma, obtained by using the function pd_RW_gamma"
      )
    }
    if (gamma_sampler$m != m) {
      stop(
        "The number of states specified in m is not equal to the number of states specified when setting the proposal distribution of the RW Metropolis sampler on gamma using the function pd_RW_gamma"
      )
    }
  ...
  if (data_distr == 'categorical') {
    ...
    if (is.null(emiss_sampler)) {
      ...
      if (!is.mHMM_pdRW_emiss(emiss_sampler)) {
        stop(
          "The input object specified for emiss_sampler should be from the class mHMM_pdRW_emiss, obtained by using the function pd_RW_emiss_cat"
        )
      }
      if (emiss_sampler$gen$m != m) {
        stop(
          "The number of states specified in m is not equal to the number of states specified when setting the proposal distribution of the RW Metropolis sampler on the emission distribution(s) using the function pd_RW_emiss_cat."
        )
      }
      if (emiss_sampler$gen$n_dep != n_dep) {
        stop(
          "The number of dependent variables specified in n_dep is not equal to the number of dependent variables specified when setting the proposal distribution of the RW Metropolis sampler on the emission distribution(s) using the function pd_RW_emiss_cat."
        )
      }
      if (sum(emiss_sampler$gen$q_emiss != q_emiss) > 0) {
        stop(
          "The number of number of observed categories for each of the dependent variable specified in q_emiss is not equal to q_emiss specified when setting the proposal distribution of the RW Metropolis sampler on the emission distribution(s) using the function pd_RW_emiss_cat."
        )
      }
    ...
    if (is.null(emiss_sampler)) {
      ...
      if (!is.mHMM_pdRW_emiss(emiss_sampler)) {
        stop(
          "The input object specified for emiss_sampler should be from the class mHMM_pdRW_emiss, obtained by using the function pd_RW_emiss_count"
        )
      }
      if (emiss_sampler$gen$m != m) {
        stop(
          "The number of states specified in m is not equal to the number of states specified when setting the proposal distribution of the RW Metropolis sampler on the emission distribution(s) using the function pd_RW_emiss_count."
        )
      }
      if (emiss_sampler$gen$n_dep != n_dep) {
        stop(
          "The number of dependent variables specified in n_dep is not equal to the number of dependent variables specified when setting the proposal distribution of the RW Metropolis sampler on the emission distribution(s) using the function pd_RW_emiss_count."
        )
      }
      emiss_scalar <- emiss_sampler$emiss_scalar
      emiss_w <- emiss_sampler$emiss_w
  ...
  if (is.null(gamma_hyp_prior)) {
    ...
    if (!is.mHMM_prior_gamma(gamma_hyp_prior)) {
      stop(
        "The input object specified for gamma_hyp_prior should be from the class mHMM_prior_gamma, obtained by using the function prior_gamma."
      )
    }
    if (gamma_hyp_prior$m != m) {
      stop(
        "The number of states specified in m is not equal to the number of states specified when creating the informative hper-prior distribution gamma using the function prior_gamma."
      )
    }
    if (is.null(gamma_hyp_prior$n_xx_gamma) & nx[1] > 1) {
      stop(
        "Covariates were specified to predict gamma, but no covariates were specified when creating the informative hyper-prior distribution on gamma using the function prior_gamma."
      )
    }
    if (!is.null(gamma_hyp_prior$n_xx_gamma)) {
      if (gamma_hyp_prior$n_xx_gamma != (nx[1] - 1)) {
        stop(
          "The number of covariates specified to predict gamma is not equal to the number of covariates specified when creating the informative hper-prior distribution on gamma using the function prior_gamma."
        )
      }
    }
  ...
  if (data_distr == 'categorical') {
    if (is.null(emiss_hyp_prior)) {
      ...
      if (!is.mHMM_prior_emiss(emiss_hyp_prior) | !is.cat(emiss_hyp_prior)) {
        stop(
          "The input object specified for emiss_hyp_prior should be from the class mHMM_prior_emiss, obtained by using the function for categorical data: prior_emiss_cat."
        )
      }
      if (emiss_hyp_prior$gen$m != m) {
        stop(
          "The number of states specified in m is not equal to the number of states specified when creating the informative hper-prior distribution on the emission distribution(s) using the function prior_emiss_cat."
        )
      }
      if (emiss_hyp_prior$gen$n_dep != n_dep) {
        stop(
          "The number of dependent variables specified in n_dep is not equal to the number of dependent variables specified when creating the informative hper-prior distribution on the emission distribution(s) using the function prior_emiss_cat."
        )
      }
      if (sum(emiss_hyp_prior$gen$q_emiss != q_emiss) > 0) {
        stop(
          "The number of number of observed categories for each of the dependent variable specified in q_emiss is not equal to q_emiss specified when creating the informative hper-prior distribution on the emission distribution(s) using the function prior_emiss_cat."
        )
      }
      if (is.null(emiss_hyp_prior$n_xx_emiss) & sum(nx[2:n_dep1] > 1) > 0) {
        stop(
          "Covariates were specified to predict the emission distribution(s), but no covariates were specified when creating the informative hyper-prior distribution on the emission distribution(s) using the function prior_emiss_cat."
        )
      }
      if (!is.null(emiss_hyp_prior$n_xx_emiss)) {
        if (sum(emiss_hyp_prior$n_xx_emiss != (nx[2:n_dep1] - 1)) > 0) {
          stop(
            "The number of covariates specified to predict the emission distribution(s) is not equal to the number of covariates specified when creating the informative hper-prior distribution on the emission distribution(s) using the function prior_emiss_cat."
          )
        }
      }
    ...
    if (missing(emiss_hyp_prior)) {
      stop(
        "The hyper-prior values for the Normal emission distribution(s) denoted by emiss_hyp_prior needs to be specified"
      )
    }
    if (!is.mHMM_prior_emiss(emiss_hyp_prior) | !is.cont(emiss_hyp_prior)) {
      stop(
        "The input object specified for emiss_hyp_prior should be from the class mHMM_prior_emiss, obtained by using the function for continuous data: prior_emiss_cont."
      )
    }
    if (emiss_hyp_prior$gen$m != m) {
      stop(
        "The number of states specified in m is not equal to the number of states specified when creating the informative hper-prior distribution on the emission distribution(s) using the function prior_emiss_cont."
      )
    }
    if (emiss_hyp_prior$gen$n_dep != n_dep) {
      stop(
        "The number of dependent variables specified in n_dep is not equal to the number of dependent variables specified when creating the informative hper-prior distribution on the emission distribution(s) using the function prior_emiss_cont."
      )
    }
    if (is.null(emiss_hyp_prior$n_xx_emiss) & sum(nx[2:n_dep1] > 1) > 0) {
      stop(
        "Covariates were specified to predict the emission distribution(s), but no covariates were specified when creating the informative hyper-prior distribution on the emission distribution(s) using the function prior_emiss_cont."
      )
    }
    if (!is.null(emiss_hyp_prior$n_xx_emiss)) {
      if (sum(emiss_hyp_prior$n_xx_emiss != (nx[2:n_dep1] - 1)) > 0) {
  ...
  if (data_distr == 'categorical') {
    emiss_int_mle_pooled <- emiss_pooled_ll <- rep(
      list(vector("list", n_dep)),
      m
    )
    ...
    emiss_c_mu <- rep(
      list(rep(list(matrix(NA_real_, ncol = 1, nrow = n_subj)), n_dep)),
      m
    )
    for (i in 1:m) {
      for (q in 1:n_dep) {
    ...
    emiss_V_mu <- emiss_c_mu_bar <- emiss_c_V <- rep(
      list(rep(list(NULL), n_dep)),
      m
    )
  ...
  if (length(start_val) != n_dep + 1 | depth(start_val) != 1) {
    stop(
      "The number of elements in the list start_val should be equal to 1 + the number of dependent variables,
         and should not contain nested lists (i.e., lists within lists)"
    )
  }
  PD <- list(
    trans_prob = matrix(NA_real_, nrow = J, ncol = m * m),
    cat_emiss = if (data_distr == 'categorical') {
      matrix(NA_real_, nrow = J, ncol = sum(m * q_emiss))
    } else {
      NULL
    },
    cont_emiss = if (data_distr == 'continuous') {
      matrix(NA_real_, nrow = J, ncol = (n_dep * 2 * m))
    } else {
      NULL
    },
    count_emiss = if (data_distr == 'count') {
      matrix(NA_real_, nrow = J, ncol = (n_dep * m))
    } else {
      NULL
    },
  ...
  if (dim(start_val[[1]])[1] != m | dim(start_val[[1]])[2] != m) {
    stop(paste0(
      "Start values for the transition probability matrix contained in the first element of 'start_val' should be an m x m matrix, here ",
      m,
      " by ",
      m,
      "."
  ...
  if (data_distr == 'categorical') {
    if (
      sum(sapply(start_val, dim)[1, 2:(n_dep + 1)] != rep(m, n_dep)) > 0 |
        sum(sapply(start_val, dim)[2, 2:(n_dep + 1)] != q_emiss) > 0
    ) {
      stop(
        "Start values for the emission distribuitons for categorical observations contained in 'start_val' should be one matrix per dependent variable, each with m rows and q_emiss[k] columns"
      )
    }
    PD_cat_emiss_names <- character()
    for (q in 1:n_dep) {
      PD_cat_emiss_names <- c(
        PD_cat_emiss_names,
        paste(
          "dep",
          q,
          "_S",
          rep(1:m, each = q_emiss[q]),
          "_emiss",
          rep(1:q_emiss[q], m),
          sep = ""
        )
    ...
    for (q in 1:n_dep) {
      PD$cat_emiss[
        1,
        (sum(c(0, q_emiss)[1:q] * m) + 1):sum(q_emiss[1:q] * m)
      ] <- as.vector(t(start_val[[q + 1]]))
    }
  } else if (data_distr == 'continuous') {
    if (
      sum(sapply(start_val, dim)[1, 2:(n_dep + 1)] != rep(m, n_dep)) > 0 |
      ...
      stop(
        "Start values for the emission distribuitons for continuous observations contained in 'start_val' should be one matrix per dependent variable, each with m rows and 2 columns"
      )
    }
    colnames(PD$cont_emiss) <- c(
      paste("dep", rep(1:n_dep, each = m), "_S", rep(1:m), "_mu", sep = ""),
    ...
    for (q in 1:n_dep) {
      ...
      PD$cont_emiss[
        1,
        (n_dep * m + (q - 1) * m + 1):(n_dep * m + q * m)
      ] <- start_val[[q + 1]][, 2]^2
    }
  } else if (data_distr == 'count') {
    if (sum(sapply(start_val, dim)[1, 2:(n_dep + 1)] != rep(m, n_dep)) > 0) {
      stop(
        "Start values for the emission distribuitons for continuous observations contained in 'start_val' should be one matrix per dependent variable, each with m rows and 1 column"
      )
    }
    colnames(PD$count_emiss) <- c(paste(
      "dep",
      rep(1:n_dep, each = m),
      "_S",
      rep(1:m),
  ...
  for (i in 1:n_subj) {
    # add frequency table for local decoding
  ...
  gamma_int_bar[1, ] <- as.vector(t(prob_to_int(matrix(
    gamma_prob_bar[1, ],
    byrow = TRUE,
    ncol = m,
    nrow = m
  ...
  colnames(gamma_V_int_bar) <- paste0(
    "var_int_S",
    rep(1:m, each = m - 1),
    "toS",
    2:m
  ...
  if (nx[1] > 1) {
    gamma_cov_bar <- matrix(
      NA_real_,
      nrow = J,
      ncol = ((m - 1) * m) * (nx[1] - 1)
    )
    colnames(gamma_cov_bar) <- paste(
      paste("cov", 1:(nx[1] - 1), "_", sep = ""),
      "S",
      rep(1:m, each = (m - 1) * (nx[1] - 1)),
      "toS",
      rep(2:m, m * (nx[1] - 1)),
      sep = ""
    )
  ...
  if (data_distr == 'categorical') {
    ...
    for (q in 1:n_dep) {
      colnames(emiss_prob_bar[[q]]) <- paste(
        "Emiss",
        rep(1:q_emiss[q], m),
        "_S",
        rep(1:m, each = q_emiss[q]),
      ...
      start <- c(0, q_emiss * m)
      emiss_prob_bar[[q]][1, ] <- PD$cat_emiss[
        1,
        (sum(c(0, q_emiss)[1:q] * m) + 1):sum(q_emiss[1:q] * m)
      ]
    }
    ...
    for (q in 1:n_dep) {
      colnames(emiss_int_bar[[q]]) <- paste(
        "int_Emiss",
        rep(2:q_emiss[q], m),
        "_S",
        rep(1:m, each = q_emiss[q] - 1),
        sep = ""
      )
      emiss_int_bar[[q]][1, ] <- as.vector(t(prob_to_int(matrix(
        emiss_prob_bar[[q]][1, ],
        byrow = TRUE,
        ncol = q_emiss[q],
        nrow = m
      ))))
      colnames(emiss_V_int_bar[[q]]) <- paste(
        "var_int_Emiss",
        2:q_emiss[q],
        "_S",
        rep(1:m, each = (q_emiss[q] - 1)),
      ...
      emiss_V_idx[[q]] <- which(
        paste(
          "var_int_Emiss",
          rep(2:q_emiss[q], each = (q_emiss[q] - 1)),
          "_with_Emiss",
          rep(2:q_emiss[q], (q_emiss[q] - 1)),
          "_S",
          rep(1:m, each = (q_emiss[q] - 1) * (q_emiss[q] - 1)),
          sep = ""
        ) %in%
          paste(
            "var_int_Emiss",
    ...
    if (sum(nx[-1]) > n_dep) {
      ...
      for (q in 1:n_dep) {
        if (nx[1 + q] > 1) {
          colnames(emiss_cov_bar[[q]]) <- paste(
            paste("cov", 1:(nx[1 + q] - 1), "_", sep = ""),
            "emiss",
            rep(2:q_emiss[q], m * (nx[1 + q] - 1)),
            "_S",
            rep(1:m, each = (q_emiss[q] - 1) * (nx[1 + q] - 1)),
            sep = ""
          )
          emiss_cov_bar[[q]][1, ] <- 0
        } else {
          emiss_cov_bar[[
            q
          ]] <- "No covariates where used to predict the emission probabilities for this outcome"
        }
      }
    } else {
    ...
    emiss_mu_bar <- rep(
      list(matrix(
        ...
        ncol = m,
        nrow = J,
        dimnames = list(NULL, c(paste("mu_", 1:m, sep = "")))
      )),
      n_dep
    )
    ...
    emiss_varmu_bar <- rep(
      list(matrix(
        ...
        ncol = m,
        nrow = J,
        dimnames = list(NULL, c(paste("varmu_", 1:m, sep = "")))
      )),
      n_dep
    )
    ...
    emiss_sd_bar <- rep(
      list(matrix(
        ...
        ncol = m,
        nrow = J,
        dimnames = list(NULL, c(paste("sd_", 1:m, sep = "")))
      )),
      n_dep
    )
    names(emiss_sd_bar) <- dep_labels
    for (q in 1:n_dep) {
      emiss_sd_bar[[q]][1, ] <- PD$cont_emiss[
        1,
        (n_dep * m + (q - 1) * m + 1):(n_dep * m + q * m)
      ]
    }
    ...
    emiss_logmu_bar <- rep(
      list(matrix(
        ...
        ncol = m,
        nrow = J,
        dimnames = list(NULL, c(paste("logmu_", 1:m, sep = "")))
      )),
      n_dep
    )
    names(emiss_mu_bar) <- dep_labels
    ...
    emiss_logvarmu_bar <- rep(
      list(matrix(
        NA_real_,
        ncol = m,
        nrow = J,
        dimnames = list(NULL, c(paste("logvarmu_", 1:m, sep = "")))
      )),
      n_dep
    )
    names(emiss_varmu_bar) <- dep_labels
  ...
  if (data_distr == 'categorical') {
    if (sum(unlist(start_val[2:(n_dep + 1)]) == 0) > 0) {
      message(
        "The starting vaules for your categorical emission distribution(s) contain values equalling zero.
  ...
  if (sum(start_val[[1]] == 0) > 0) {
    message(
      "The starting vaules for your transition probability matrix gamma contains values equalling zero.
              This can cause estimation problems, please consider using (very small) non-zero values instead."
    )
  }
  ...
  if (relabel_type == "observed") {
    ...
    for (s in 1:n_subj) {
      is_observed[[s]] <- which(
        !apply(
          is.na(s_data[s_data[, 1] == unique_ID[s], -1, drop = FALSE]),
          1,
          any
        )
      )
  ...
  for (iter in 2:J) {
    ...
    for (s in 1:n_subj) {
      ...
      if (data_distr == 'categorical') {
        ...
        forward <- count_mult_fw_r_to_cpp(
          x = subj_data[[s]]$y,
          m = m,
          emiss = emiss[[s]],
          gamma = gamma[[s]],
      ...
      if (sum(is.nan(alpha)) > 0) {
        if (iter == 2) {
          if (data_distr == 'continuous') {
            stop(
              "The forward-backward algorithm ran into a fatal error during the first MCMC iteration. Most likely, starting values that do not match the data
      ...
      sample_path[[s]][n_vary[[s]], iter] <- sample(
        1:m,
        1,
        prob = c(alpha[, n_vary[[s]]])
      )
      for (t in (subj_data[[s]]$n_t - 1):1) {
        sample_path[[s]][t, iter] <- sample(
          1:m,
          1,
          prob = (alpha[, t] * gamma[[s]][, sample_path[[s]][t + 1, iter]])
        )
      }
      samp_seq <- sample_path[[s]][, iter]
      relab <- NULL
      if (
        iter >= start_relabeling &
          (((iter - start_relabeling) %% relabel_steps) == 0)
      ) {
        ## relabel sampled sequence
        if (relabel_type == "all") {
          # if also relabel unobserved
          pivot <- max.col(
            PD_subj[[s]]$sampled_state_freq,
            ties.method = "random"
          ) # create pivot
          ...
          # if only relabel points with observed data
          pivot <- max.col(
            PD_subj[[s]]$sampled_state_freq[is_observed[[s]], ],
            ties.method = "random"
          ) # create pivot using local decoding
          relab <- ecr_observed(
            pivot,
            sample_path[[s]][, iter],
            is_observed[[s]],
        ...
        if (isTRUE(relab$switched)) {
          ## relabel the previous iteration's parameter values in the same
          ## fashion, so the samplers continue from the permuted state instead
          ## of the state belonging to the old labeling. ecr() returns the new
          ## label of old state i, so invert it to get the old state that
          ## becomes new state i. Same procedure as used in mHMM_relabel_pra.
          permute <- order(relab$permutation)
          gamma_relabeled <- int_to_prob_noround(matrix(
            unlist(lapply(gamma_c_int, function(e) e[s, ])),
            nrow = m,
            byrow = TRUE
          ))[permute, permute]
          gamma_relabeled_int <- prob_to_int(gamma_relabeled)
          # the emission samplers are indexed by state, so a plain reordering
          emiss_c_old <- if (data_distr == 'categorical') {
            emiss_c_int
          } else {
            emiss_c_mu
          }
          for (i in 1:m) {
            ...
            for (q in 1:n_dep) {
              if (data_distr == 'categorical') {
                emiss_c_int[[i]][[q]][s, ] <- emiss_c_old[[permute[i]]][[q]][s, ]
              } else {
                emiss_c_mu[[i]][[q]][s, 1] <- emiss_c_old[[permute[i]]][[q]][s, 1]
              }
            }
          }
        }
      ...
      if (iter > relabel_burnin) {
        # update pivot
        PD_subj[[s]]$sampled_state_freq[cbind(
          1:n_vary[[s]],
          samp_seq
        )] <- PD_subj[[s]]$sampled_state_freq[cbind(1:n_vary[[s]], samp_seq)] +
          1 # update local decoding
      }
      for (t in (subj_data[[s]]$n_t - 1):1) {
        trans[[s]][[sample_path[[s]][t, iter]]] <- c(
          trans[[s]][[sample_path[[s]][t, iter]]],
          sample_path[[s]][t + 1, iter]
        )
      }
      for (i in 1:m) {
        ...
        for (q in 1:n_dep) {
          cond_y[[s]][[q]][[i]] <- stats::na.omit(c(subj_data[[s]]$y[
            sample_path[[s]][, iter] == i,
            q
          ]))
        }
      }
    ...
    for (i in 1:m) {
      ...
      if (data_distr == 'categorical') {
        for (q in 1:n_dep) {
          cond_y_pooled <- c(
            unlist(sapply(sapply(cond_y, "[[", q, simplify = FALSE), "[[", i)),
            c(1:q_emiss[q])
          )
          emiss_mle_pooled <- optim(
            emiss_int_mle0[[q]][i, ],
            llmnl_int,
            Obs = cond_y_pooled,
            n_cat = q_emiss[q],
        ...
        for (q in 1:n_dep) {
          ...
          emiss_mle_pooled[[i]][[q]] <- mean(
            c(cond_y_pooled, round(exp(emiss_c_mu_bar[[i]][[q]][1]), 0)),
            na.rm = TRUE
          )
          if (emiss_mle_pooled[[i]][[q]] == 0) {
            emiss_mle_pooled[[i]][[q]] <- 0.01
          }
          emiss_pooled_ll[[i]][[q]] <- llpois(
            lambda = emiss_mle_pooled[[i]][[q]],
            Obs = c(cond_y_pooled, round(exp(emiss_c_mu_bar[[i]][[q]][1]), 0))
          )
        }
      }
      ...
      for (s in 1:n_subj) {
        ...
        gamma_out <- optim(
          gamma_int_mle_pooled[[i]],
          llmnl_int_frac,
          Obs = c(trans[[s]][[i]], c(1:m)),
          n_cat = m,
          pooled_likel = gamma_pooled_ll[[i]],
          w = gamma_w,
          wgt = wgt,
          method = "BFGS",
          hessian = FALSE,
        ...
        if (gamma_out$convergence == 0) {
          subj_data[[s]]$gamma_mhess[
            (1 + (i - 1) * (m - 1)):((m - 1) + (i - 1) * (m - 1)),
          ] <-
            mnlHess_int(
              int = gamma_out$par,
              Obs = c(trans[[s]][[i]], c(1:m)),
        ...
        if (data_distr == 'categorical') {
          for (q in 1:n_dep) {
            emiss_out <- optim(
              emiss_int_mle_pooled[[i]][[q]],
              llmnl_int_frac,
              Obs = c(cond_y[[s]][[q]][[i]], c(1:q_emiss[q])),
              n_cat = q_emiss[q],
            ...
            if (emiss_out$convergence == 0) {
              subj_data[[s]]$emiss_mhess[[q]][
                (1 + (i - 1) * (q_emiss[q] - 1)):((q_emiss[q] - 1) +
                  (i - 1) * (q_emiss[q] - 1)),
              ] <-
                mnlHess_int(
                  int = emiss_out$par,
          ...
          for (q in 1:n_dep) {
            emiss_out <- optim(
              log(emiss_mle_pooled[[i]][[q]]),
              llpois_frac_log,
              Obs = c(
                cond_y[[s]][[q]][[i]],
                round(exp(emiss_c_mu_bar[[i]][[q]][1]), 0)
              ),
              pooled_likel = emiss_pooled_ll[[i]][[q]],
              w = emiss_w[[q]],
              wgt = wgt,
            ...
            if (emiss_out$convergence == 0) {
              ...
              subj_data[[s]]$emiss_mhess[[q]][i] <- as.numeric(
                -emiss_out$hessian
              )
            } else {
      ...
      gamma_mu0_n <- solve(t(xx[[1]]) %*% xx[[1]] + gamma_K0) %*%
        (t(xx[[1]]) %*% gamma_c_int[[i]] + gamma_K0 %*% gamma_mu0[[i]])
      gamma_V_n <- gamma_V +
        t(gamma_c_int[[i]] - xx[[1]] %*% gamma_mu0_n) %*%
          (gamma_c_int[[i]] - xx[[1]] %*% gamma_mu0_n) +
        t(gamma_mu0_n - gamma_mu0[[i]]) %*%
          gamma_K0 %*%
          (gamma_mu0_n - gamma_mu0[[i]])
      gamma_V_int[[i]] <- solve(rwish(
        S = solve(gamma_V_n),
        v = gamma_nu + n_subj
      ))
      gamma_mu_int_bar[[i]] <- gamma_mu0_n +
      ...
      gamma_mu_prob_bar[[i]] <- gamma_exp_int /
        as.vector(gamma_exp_int %*% c(rep(1, (m))))

      if (data_distr == 'categorical') {
        for (q in 1:n_dep) {
          emiss_mu0_n <- solve(
            t(xx[[1 + q]]) %*% xx[[1 + q]] + emiss_K0[[q]]
          ) %*%
            (t(xx[[1 + q]]) %*%
              emiss_c_int[[i]][[q]] +
          ...
          emiss_V_n <- emiss_V[[q]] +
            ...
            t(emiss_mu0_n - emiss_mu0[[q]][[i]]) %*%
              emiss_K0[[q]] %*%
              (emiss_mu0_n - emiss_mu0[[q]][[i]])
          emiss_V_int[[i]][[q]] <- solve(rwish(
            S = solve(emiss_V_n),
            v = emiss_nu[[q]] + n_subj
          ))
          emiss_mu_int_bar[[i]][[q]] <- emiss_mu0_n +
            solve(chol(t(xx[[1 + q]]) %*% xx[[1 + q]] + emiss_K0[[q]])) %*%
              matrix(rnorm((q_emiss[q] - 1) * nx[1 + q]), nrow = nx[1 + q]) %*%
              t(solve(chol(solve(emiss_V_int[[i]][[q]]))))
          emiss_exp_int <- matrix(
            exp(c(0, emiss_mu_int_bar[[i]][[q]][1, ])),
            nrow = 1
          )
          emiss_mu_prob_bar[[i]][[q]] <- emiss_exp_int /
            as.vector(emiss_exp_int %*% c(rep(1, (q_emiss[q]))))
        }
      } else if (data_distr == 'continuous') {
        ...
        for (q in 1:n_dep) {
          ...
          emiss_b_mu_n <- (emiss_nu[[q]] * emiss_V[[q]][i]) /
            2 +
            (t(emiss_c_mu[[i]][[q]]) %*%
              emiss_c_mu[[i]][[q]] +
              ...
              t(emiss_mu0_n) %*%
                ...
                emiss_mu0_n) /
              2
          emiss_V_mu[[i]][[q]] <- as.numeric(solve(stats::rgamma(
            1,
            shape = emiss_a_mu_n,
            rate = emiss_b_mu_n
          )))
          emiss_c_mu_bar[[i]][[q]] <- emiss_mu0_n +
            rnorm(
              1 + nx[1 + q] - 1,
              mean = 0,
              sd = sqrt(diag(
      ...
      for (s in 1:n_subj) {
        gamma_candcov_comb <- chol2inv(chol(
          subj_data[[s]]$gamma_mhess[
            (1 + (i - 1) * (m - 1)):((m - 1) + (i - 1) * (m - 1)),
          ] +
            chol2inv(chol(gamma_V_int[[i]]))
        ))
        gamma_RWout <- mnl_RW_once(
          int1 = gamma_c_int[[i]][s, ],
          Obs = trans[[s]][[i]],
          n_cat = m,
        ...
        gamma[[s]][i, ] <- PD_subj[[s]]$trans_prob[
          iter,
          ((i - 1) * m + 1):((i - 1) * m + m)
        ] <- (gamma_RWout$prob + .0001) / (1 + 0.0001 * m)
      ...
      if (data_distr == 'categorical') {
        for (s in 1:n_subj) {
          start <- c(0, q_emiss * m)
          for (q in 1:n_dep) {
            emiss_candcov_comb <- chol2inv(chol(
              subj_data[[s]]$emiss_mhess[[q]][
                (1 + (i - 1) * (q_emiss[q] - 1)):((q_emiss[q] - 1) +
                  (i - 1) * (q_emiss[q] - 1)),
              ] +
                chol2inv(chol(emiss_V_int[[i]][[q]]))
            ))
            emiss_RWout <- mnl_RW_once(
              int1 = emiss_c_int[[i]][[q]][s, ],
              Obs = cond_y[[s]][[q]][[i]],
              n_cat = q_emiss[q],
              mu_int_bar1 = c(
                t(emiss_mu_int_bar[[i]][[q]]) %*% xx[[1 + q]][s, ]
              ),
              V_int1 = emiss_V_int[[i]][[q]],
              scalar = emiss_scalar[[q]],
              candcov1 = emiss_candcov_comb
            )
            emiss[[s]][[q]][i, ] <- PD_subj[[s]]$cat_emiss[
              iter,
              (sum(start[1:q]) + 1 + (i - 1) * q_emiss[q]):(sum(start[1:q]) +
                (i - 1) * q_emiss[q] +
                q_emiss[q])
            ] <- (emiss_RWout$prob + .0001) / (1 + 0.0001 * q_emiss[q])
            emiss_naccept[[q]][s, i] <- emiss_naccept[[q]][s, i] +
              emiss_RWout$accept
        ...
        for (q in 1:n_dep) {
          for (s in 1:n_subj) {
            ss_subj[s] <- t(
              matrix(
                cond_y[[s]][[q]][[i]] - emiss_c_mu[[i]][[q]][s, 1],
                nrow = 1
              ) %*%
                matrix(
                  cond_y[[s]][[q]][[i]] - emiss_c_mu[[i]][[q]][s, 1],
          ...
          emiss_c_V[[i]][[q]] <- solve(stats::rgamma(
            1,
            shape = emiss_a_resvar_n,
            rate = emiss_b_resvar_n
          ))
        ...
        for (q in 1:n_dep) {
          emiss_c_V_subj <- (emiss_V_mu[[i]][[q]] * emiss_c_V[[i]][[q]]) /
            (2 * emiss_V_mu[[i]][[q]] + emiss_c_V[[i]][[q]])
          for (s in 1:n_subj) {
            emiss_mu0_subj_n <- (emiss_V_mu[[i]][[q]] *
              sum(cond_y[[s]][[q]][[i]]) +
              emiss_c_V[[i]][[q]] *
                c(t(emiss_c_mu_bar[[i]][[q]]) %*% xx[[q + 1]][s, ])) /
              (n_cond_y[s] * emiss_V_mu[[i]][[q]] + emiss_c_V[[i]][[q]])
            emiss[[s]][[q]][i, 1] <- PD_subj[[s]]$cont_emiss[
              iter,
              ((q - 1) * m + i)
            ] <- emiss_c_mu[[i]][[q]][s, 1] <- rnorm(
              1,
              emiss_mu0_subj_n,
              sqrt(emiss_c_V_subj)
            )
            emiss[[s]][[q]][i, 2] <- emiss_c_V[[i]][[q]]
            PD_subj[[s]]$cont_emiss[
              iter,
              (n_dep * m + (q - 1) * m + i)
            ] <- sqrt(emiss_c_V[[i]][[q]])
          }
        ...
        for (q in 1:n_dep) {
          for (s in 1:n_subj) {
            ...
            emiss_mu0_subj_bar <- c(
              t(emiss_c_mu_bar[[i]][[q]]) %*% xx[[q + 1]][s, ]
            )
            emiss_candcov_comb <- (subj_data[[s]]$emiss_mhess[[q]][i] +
              emiss_V_mu[[i]][[q]]^-1)^-1
            emiss_rw_out <- pois_RW_once(
              lambda = emiss_c_mu[[i]][[q]][s, 1],
              Obs = cond_y[[s]][[q]][[i]],
            ...
            emiss[[s]][[q]][i, 1] <- PD_subj[[s]]$count_emiss[
              iter,
              ((q - 1) * m + i)
            ] <- emiss_c_mu[[i]][[q]][s, 1] <- emiss_rw_out$draw_lambda
            emiss_naccept[[q]][s, i] <- emiss_naccept[[q]][s, i] +
              emiss_rw_out$accept
          }
        }
    ...
    gamma_V_int_bar[iter, ] <- unlist(lapply(gamma_V_int, function(e) {
      as.vector(t(e))
    }))[gamma_V_idx]
    ...
    if (data_distr == 'categorical') {
      for (q in 1:n_dep) {
        emiss_int_bar[[q]][iter, ] <- as.vector(unlist(lapply(
          lapply(emiss_mu_int_bar, "[[", q),
          "[",
          1,
        )))
        emiss_V_int_bar[[q]][iter, ] <- as.vector(unlist(lapply(
          lapply(emiss_V_int, "[[", q),
          function(e) as.vector(t(e))
        )))[emiss_V_idx[[q]]]
        if (nx[1 + q] > 1) {
        ...
        emiss_prob_bar[[q]][iter, ] <- as.vector(unlist(sapply(
          emiss_mu_prob_bar,
          "[[",
          q
        )))
      ...
      for (q in 1:n_dep) {
        ...
        emiss_varmu_bar[[q]][iter, ] <- as.vector(unlist(sapply(
          emiss_V_mu,
          "[[",
          q
        )))
      ...
      if (relabel_group) {
        if (iter %in% relabel_group_iter) {
          if (iter == relabel_group_iter[1]) {
            ...
            group_emiss_mean <- apply(
              do.call('cbind', emiss_mu_bar)[(relabel_burnin + 1):iter, ],
              2,
              mean
            ) # group-level pivot
            emiss_mean_subj <- vector('list', n_subj)
            for (s in 1:n_subj) {
              emiss_mean_subj[[s]] <- apply(
                PD_subj[[s]]$cont_emiss[
                  ((relabel_burnin + 1):iter),
                  1:(n_dep * m)
                ],
                2,
                mean
            ...
            group_emiss_mean <- group_emiss_mean +
              (unlist(lapply(emiss_mu_bar, '[', iter, 1:m)) -
                group_emiss_mean) /
                group_emiss_mean_denom # update group-level pivot
            for (s in 1:n_subj) {
              emiss_mean_subj[[s]] <- emiss_mean_subj[[s]] +
                (PD_subj[[s]]$cont_emiss[iter, 1:(n_dep * m)] -
                  emiss_mean_subj[[s]]) /
                  group_emiss_mean_denom # update subject-level parameter vector
            }
          }
          for (s in 1:n_subj) {
            ...
            emiss_mean_subj[[s]] <- c(relab_group_results$param_emiss_relabel)
          }
        }
      }
    } else if (data_distr == 'count') {
      for (q in 1:n_dep) {
        ...
        emiss_varmu_bar[[q]][iter, ] <- logvar_to_var(
          emiss_logmu_bar[[q]][iter, ],
          as.vector(unlist(sapply(emiss_V_mu, "[[", q)))
        )
        emiss_logvarmu_bar[[q]][iter, ] <- as.vector(unlist(sapply(
          emiss_V_mu,
          "[[",
          q
        )))
    ...
    if (return_path == TRUE) {
      out <- list(
        input = list(
          ...
          n_vary = n_vary,
          dep_labels = dep_labels,
          relabel_train = relabel_train,
          relabel_steps = relabel_steps
        ),
        PD_subj = PD_subj,
        gamma_int_subj = gamma_int_subj,
        gamma_int_bar = gamma_int_bar,
